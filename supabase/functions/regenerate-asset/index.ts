...
const ASSET_TITLES: Record<AssetType, string> = {
  ...
  product_hunt_copy: "Product Hunt Copy",
...
const ROCKET_PERSONA =
  "You are Rocket — an AI branding system for startups. You only create startup branding assets. Return only the requested asset, with no preamble.";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if ([429, 500, 502, 503, 504].includes(response.status))
        throw new GeminiUnavailableError(`Gemini ${response.status}: ${detail}`);
    ...
    if (error instanceof DOMException && error.name === "AbortError")
      throw new GeminiUnavailableError("Gemini request timed out");
    throw error;
  } finally {
  ...
  if (OPENAI_API_KEY) {
    try {
      ...
      if (response.ok && data?.data?.[0]?.b64_json)
        return Uint8Array.from(atob(data.data[0].b64_json), (char) => char.charCodeAt(0));
    } catch (error) {
...
const GENERATORS: Record<
  AssetType,
  {
    ...
    system: string;
    json?: boolean;
    build: (context: Record<string, unknown>, instruction: string) => string;
  }
> = Object.fromEntries(
  Object.keys(ASSET_TITLES).map((type) => [
    type,
    {
...
Deno.serve(async (req) => {
  ...
  try {
    if (!hasGeminiKey())
      return new Response(JSON.stringify({ error: "missing_environment_variable", variable: "GEMINI_API_KEY" }), {
        status: 200,
    ...
    if (!reserved)
      return new Response(JSON.stringify({ error: "no_credits", code: "no_credits", needed: cost }), {
        status: 200,
        headers: { ...ch, "Content-Type": "application/json" },
      });
    ...
    try {
      if (spec.kind === "image") {
        const imgPrompt = await geminiText({
          system: spec.system,
          user: spec.build(ctx, userPrompt),
          temperature: 0.9,
        });
        ...
        await admin
          ...
          .update({
            ...
            meta: { ...(asset.meta || {}), image_prompt: imgPrompt },
          })
          .eq("id", asset_id);
        reservationOpen = false;
        return new Response(JSON.stringify({ image_url: pub.publicUrl, credits_charged: cost }), {
          headers: { ...ch, "Content-Type": "application/json" },
        });
      } else {
        ...
        reservationOpen = false;
        return new Response(JSON.stringify({ content, credits_charged: cost }), {
          headers: { ...ch, "Content-Type": "application/json" },
        });
      }
    } catch (e) {
